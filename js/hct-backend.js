...
(function () {
  ...
  async function loadSharedState() {
    ...
    try {
      ...
      // Purge GLOBAL_ALERTS for discharged patients (not in active patient stores)
      if (Array.isArray(window.GLOBAL_ALERTS)) {
        var activeIds = new Set();
        [window.PATIENTS, window.OPD_PATIENTS, window.LTC_PATIENTS].forEach(function(store) {
          if (!store) return;
          Object.values(store).forEach(function(ward) {
            (ward || []).forEach(function(p) { if (p && p.id) activeIds.add(p.id); });
          });
        });
        var beforeLen = window.GLOBAL_ALERTS.length;
        window.GLOBAL_ALERTS = window.GLOBAL_ALERTS.filter(function(a) {
          return !a.pxId || activeIds.has(a.pxId);
        });
        if (window.GLOBAL_ALERTS.length < beforeLen) {
          lastSharedSnap['GLOBAL_ALERTS|__global'] = null;
          setTimeout(saveSharedState, 1000);
        }
      }
  ...
  function dbRowToPx(row) {
    ...
    return {
      ...
      chiefComplaint: extra.chiefComplaint || null,
  ...
  function pxToDbRow(px, sectionType, wardId) {
    return {
      ...
      photo: px.photo || null, extra: { chiefComplaint: px.chiefComplaint || null },
  ...
  window.hctDischargePatient = async function(pxId) {
    ...
    try {
      ...
      if (res.error) { console.warn('[HCT] discharge patient error:', res.error.message); return; }
      await sb.from('ehr_notifications').delete().eq('px_id', pxId);
  ...
  async function loadNotifications() {
    ...
    try {
      ...
      // Purge notifications for discharged patients (not present in active patient stores)
      var activeIds = new Set();
      [window.PATIENTS, window.OPD_PATIENTS, window.LTC_PATIENTS].forEach(function(store) {
        if (!store) return;
        Object.values(store).forEach(function(ward) {
      ...
      var staleDbIds = [];
      window.NOTIF_LOG = (window.NOTIF_LOG || []).filter(function(n) {
        if (!n.pxId) return true;
        if (activeIds.has(n.pxId)) return true;
        if (n._dbId) staleDbIds.push(n._dbId);
        return false;
      });
      if (staleDbIds.length) {
        sb.from('ehr_notifications').delete().in('id', staleDbIds).then(function(){}).catch(function(){});
      }
  ...
  function handleIncomingChange(p) {
    ...
    if (key === 'ALERT_STORE' || key === 'GLOBAL_ALERTS') {
      try { if (typeof updateAlertBadge === 'function') updateAlertBadge(); } catch(e){}
      try { if (typeof renderAlertPanel === 'function') renderAlertPanel(); } catch(e){}
    }
  ...
  function handleSharedStateChange(payload) {
    ...
    try { if (typeof init === 'function') init(); } catch(e){}
    setSyncBadge('saved');
    if (row.state_key === 'ALERT_STORE' || row.state_key === 'GLOBAL_ALERTS') {
      try { if (typeof updateAlertBadge === 'function') updateAlertBadge(); } catch(e){}
      try { if (typeof renderAlertPanel === 'function') renderAlertPanel(); } catch(e){}
    }
  ...
  function handlePatientDBChange(payload) {
    ...
    if (row.is_discharged || row.deleted_at) {
      ...
      var pid = row.id;
      if (window.GLOBAL_ALERTS) window.GLOBAL_ALERTS = window.GLOBAL_ALERTS.filter(function(a){ return a.pxId !== pid; });
      if (window.ALERT_STORE) delete window.ALERT_STORE[pid];
      if (window.ALERT_PREV_COUNT) delete window.ALERT_PREV_COUNT[pid];
      if (window.NOTIF_LOG) window.NOTIF_LOG = window.NOTIF_LOG.filter(function(n){ return n.pxId !== pid; });
      try { if (typeof updateAlertBadge === 'function') updateAlertBadge(); } catch(e){}
      try { if (typeof updateNotifBadge === 'function') updateNotifBadge(); } catch(e){}
      try { if (typeof renderAlertPanel === 'function') renderAlertPanel(); } catch(e){}
      try { if (typeof renderNotifList === 'function') renderNotifList(); } catch(e){}
  ...
  function handleNotificationDBChange(payload) {
    ...
    if (row.created_by === uid) {
      // Link the DB id to the matching local NOTIF_LOG entry so that
      // hctMarkAllNotifsRead can persist the read status for own notifications.
      var local = (window.NOTIF_LOG || []).find(function(n) {
        return !n._dbId && n.summary === row.message &&
               n.section === (row.section || row.store_key || 'Update');
      });
      if (local) local._dbId = row.id;
      return;
    }
    injectNotifFromDB(row);
    updateNotifBadge_safe();
  ...
  function applyUserToUI() {
    ...
    try {
      var savedPhoto = localStorage.getItem('hct_avatar_data');
      if (savedPhoto) {
        window.profilePhotoData = savedPhoto;
        var avEl = document.getElementById('tb-avatar-el');
        if (avEl) avEl.innerHTML = '<img src="'+savedPhoto+'" style="width:100%;height:100%;object-fit:cover;border-radius:50%"/>';
        var avDisp = document.getElementById('profile-avatar-display');
        if (avDisp) avDisp.innerHTML = '<img src="'+savedPhoto+'" style="width:100%;height:100%;object-fit:cover"/>';
      }
    } catch(e) {}
  ...
  async function enterApp() {
    ...
    if (typeof window.hctMaybeStartTour === 'function') window.hctMaybeStartTour();
  }
  ...
  window.doSignup = async function() {
    ...
    var idno    = role==='faculty' ? (($('reg-facid')||{}).value||'').trim() : (($('reg-idno')||{}).value||'').trim();
    var section = role==='faculty' ? '' : (($('reg-section')||{}).value||'').trim();
    var school  = (($('reg-school')||{}).value||'').trim();
    var dept    = role==='faculty' ? (($('reg-dept')||{}).value||'').trim() : '';
    var p1      = ($('reg-password')||{}).value||'';
    var p2      = ($('reg-confirm')||{}).value||'';
    var consent = ($('reg-consent')||{}).checked||false;
    if (!f)          { showErr('auth-error-signup','Please enter your full name.'); return; }
    if (!email)      { showErr('auth-error-signup','Please enter your email address.'); return; }
    if (p1.length<8) { showErr('auth-error-signup','Password must be at least 8 characters.'); return; }
    if (p1!==p2)     { showErr('auth-error-signup','Passwords do not match.'); return; }
    if (!consent)    { showErr('auth-error-signup','Please read and accept the HCT Data Privacy Consent Notice to continue.'); return; }
    setBusy('signup-btn', true, 'Create account', 'Creating…');
    try {
      var res = await sb.auth.signUp({
        email:email, password:p1,
        options:{data:{full_name:f, role:role, student_no:idno||null, section:section||null, school:school||null, department:dept||null}}
      });
  ...
  /* ─────────────────────────────────────────────────────────────────────
     ...
     ───────────────────────────────────────────────────────────────────── */
  window.hctMarkAllNotifsRead = async function() {
    if (DEMO || !sb || !hctSession) return;
    var uid = hctSession.user.id;
    var log = window.NOTIF_LOG || [];
    // Collect all DB-linked notifications (do NOT filter by n.read —
    // clearAllNotifs sets read=true before calling us, so !n.read would match nothing)
    var withDbId = log.filter(function(n){ return n._dbId; });
    if (!withDbId.length) return;
    try {
      var ids = withDbId.map(function(n){ return n._dbId; });
      var res = await sb.from('ehr_notifications').select('id,read_by').in('id', ids);
      if (res.error) return;
      // Update all that don't already include this user — in parallel
      await Promise.all((res.data || [])
        .filter(function(r){
          var readBy = Array.isArray(r.read_by) ? r.read_by : [];
          return readBy.indexOf(uid) < 0;
        })
        .map(function(r){
          var readBy = (Array.isArray(r.read_by) ? r.read_by : []).concat([uid]);
          return sb.from('ehr_notifications').update({ read_by: readBy }).eq('id', r.id);
        })
      );
    } catch(e){}
  };
  ...
  /* ─────────────────────────────────────────────────────────────────────
     PROFILE SAVE
     ───────────────────────────────────────────────────────────────────── */
  window.hctSaveProfileData = async function(fullName, photoData) {
    if (!hctSession) return;
    try {
      var updates = { full_name: fullName };
      if (photoData) updates.avatar_url = photoData;
      await sb.from('profiles').update(updates).eq('id', hctSession.user.id);
      await sb.auth.updateUser({ data: { full_name: fullName } });
      if (hctProfile) hctProfile.full_name = fullName;
    } catch(e) { console.warn('[HCT] profile save error:', e); }
  };
